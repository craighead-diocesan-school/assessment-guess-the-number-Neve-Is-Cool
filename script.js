...
function guessTheNumber() {
  ...
  //Introduction to the game, asks the user for their name and places it in the variable name: usersName.

  const max = 20
  const min = 1
  let theNumber = Math.floor(Math.random() * (max - min + 1)) + min
  //Generates a random number between 1 and 20 inclusive. Sets a max and a min const of 1 to 20, allowing game to be adjusted.

  let numOfAttempts = 0
  let usersAttempt = 0
  //Sets both numOfAttempts and usersAttempt to 0 so they can be updated through the course of the game.

  alert("Hi " + usersName + "! lets play a game of guess the number! Try to guess a number between " + min + " and " + max + " inclusive in the fewest guesses possible.")
  ...
  while (usersAttempt != theNumber) {
    ...
    //Asks the user for their guess and removes additional spaces before and after the input before it is converted into a number.

    if (usersAttempt == theNumber) {
      alert("Congrats, you got it!!")
      numOfAttempts++
      boardOfScores.push("\n" + usersName + "=" + numOfAttempts)
      //When the usersAttempt is equal to the random generated number, one attempt is added to the total number of attempts and the usersName and current number of attempts is pushed to a new line within the boardOfScores array.
    } else if (usersAttempt > max || usersAttempt < min) {
      alert("Sorry, the number you guessed in not within a range of " + min + "-" + max + ", please try again")
